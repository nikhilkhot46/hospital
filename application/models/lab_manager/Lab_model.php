...
class Lab_model extends CI_Model {
	...
	public function read_by_aid($aid,$view='')
	{
		$this->db->select('*,la.discount,la.tax,la.test_price');
        $this->db->from('lab_appointments la');
		$this->db->where('la.admission_id', $aid);
		if(!$view){
			$this->db->where('la.payment_status', 0);
		}
		$this->db->join('lab_packages lp', 'lp.package_id = la.package', 'left');
		$this->db->join('lab_tests lt', 'lt.test_id = la.test', 'left');
