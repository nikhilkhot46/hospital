...
public function selected_transection($category_id)
  ...
  $this->db->select('f.*,j.firstname,j.lastname');
    $this->db->from('customer_ledger f');
    $this->db->join('patient j','f.customer_id=j.patient_id','left');
...
//loan selecete in transection update
  ...
  $this->db->select('f.*,j.firstname,j.lastname');
    ...
    $this->db->join('patient j','f.person_id=j.patient_id','left');
    $this->db->where('f.transaction_id',$trans);
    $query = $this->db->get();
    if ($query->num_rows() > 0) {
      return $query->result_array();  
...
// report popup query
  ...
  $this->db->select('
   i.*, i.amount as debit,
    i.pay_amount as credit,i.transaction_id,i.relation_id,f.firstname,f.lastname,h.supplier_name');
  $this->db->from('transection i');
  $this->db->join('patient f','i.relation_id=f.patient_id','left');
...
// todays report pop up query
    ...
    public function date_summary_query($per_page,$page)
  {
    ...
    $this->db->select('
     i.*, i.amount as debit,i.transaction_id as tranid,
...
// search query date between search by date
      public function search_query($start,$end)
  {
    $this->db->select('
      i.*, i.amount as debit,
      i.pay_amount as credit,i.transaction_id,i.relation_id,f.firstname,f.lastname,h.supplier_name,m.parent_id');
    $this->db->from('transection i');
    $this->db->join('patient f','i.relation_id=f.patient_id','left');
    $this->db->join('account_2 m','i.transection_category=m.parent_id','left');
     $this->db->join('supplier_information h','i.relation_id=h.supplier_id','left');
