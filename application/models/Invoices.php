...
class Invoices extends CI_Model {
	...
	public function invoice_list_count()
	{
		$this->db->select('a.*,b.firstname,b.lastname');
		$this->db->from('invoice a');
		$this->db->join('patient b','b.customer_id = a.customer_id');
		$this->db->order_by('a.invoice','desc');
		$this->db->limit('500');
		$query = $this->db->get();
		if ($query->num_rows() > 0) {
...
public function pos_invoice_setup($product_id){
	...
	public function pos_customer_setup(){
		$query= $this->db->select('*')
						->from('patient')
						->where('firstname','Walking Customer')
						->get();
		if ($query->num_rows() > 0) {
			return $query->result_array();	
		}
...
// product information retrieve by product id
	...
	public function invoice_by_aid($aid,$view='')
	{
		$this->db->select('id.*,i.total_discount,i.total_tax,pi.product_name');
		$this->db->from('invoice i');
		$this->db->where('i.admission_id', $aid);
		if(!$view){
			$this->db->where('i.status', 1);
			$this->db->where('id.status', 1);
			$this->db->where('id.due_amount >', 0);
		}
