	...
	*/
	...
	public function product_search_list($product_id)
	{
		...
		if ($products_list) {
			...
			redirect('dashboard_pharmacist/hospital_activities/Cproduct/manage_product');
		}
	}
	//Product Details
	...
	public function item_details($product_id)
	{
		...
		$CI->load->library('occational');
		$CI->load->model('Setting_model');
		$details_info = $CI->Item_model->item_details_info($product_id);
		$purchaseData = $CI->Item_model->item_purchase_info($product_id);
		$totalPurchase = 0;
		$totalPrcsAmnt = 0;
		...
		$salesData = $CI->Item_model->invoice_data($product_id);	
		print_r($salesData);
		$totalSales = 0;		
		$totaSalesAmt = 0;		
		if(!empty($salesData)){	
		...
		$data = array(
				'title'					=> display('product_report'),
				'product_name' 			=> $details_info[0]['item_name'],
				'purchaseTotalAmount'	=> number_format($totalPrcsAmnt, 2, '.', ','),
				'salesTotalAmount'		=> number_format($totaSalesAmt, 2, '.', ','),
				'total_purchase'		=> $totalPurchase,
				'total_sales'			=> $totalSales,
