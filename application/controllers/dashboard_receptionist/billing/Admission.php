...
class Admission extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'dashboard_receptionist/billing/admission_model' 
		));
	...
	{ 
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('dashboard_receptionist/billing/admission/index');
	...
	public function view($id = null)
	{
		...
		$data['content'] = $this->load->view('dashboard_receptionist/admission/admission_view',$data,true);
		$this->load->view('dashboard_receptionist/main_wrapper',$data);
	}
	...
	*/
	...
	{
		$patient_id = $this->input->post('admission_id');

        if (!empty($patient_id)) {
            $query = $this->db->select('ba.patient_id,p.firstname,p.lastname')
                ->from('bill_admission ba')
                ->where('ba.admission_id',$patient_id)
				->where('ba.status != ',0)
            ...
            if ($query->num_rows() > 0) {
                ...
                $data['message'] = display('invalid_admission_id');
                $data['status'] = false;
