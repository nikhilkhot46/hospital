...
class Service extends CI_Controller {
	...
	public function extra_service()
	{
        ...
        #
        $config["base_url"] = base_url('billing/service/index');
        $config["total_rows"] = $this->db->count_all('patient_service');
        $config["per_page"] = 25;
        $config["uri_segment"] = 4;
        $config["last_link"] = "Last"; 
        $config["first_link"] = "First"; 
        ...
        $this->pagination->initialize($config);
        $page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
        $data['services'] = $this->service_model->read_extra_service($config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();
        #
        #pagination ends
        #    
		$data['content'] = $this->load->view('billing/service/extra_list',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}

	public function update_servie()
	{
		...
		$data['service'] = (object)$postData = array(
			'id'           => $this->input->post('id'), 
			'patient_aid'  => $this->input->post('admission_id'), 
			'name'         => $this->input->post('name'), 
			'description'  => $this->input->post('description'), 
			'quantity'     => $this->input->post('quantity'), 
			'amount'       => $this->input->post('amount'), 
		...
		if ($this->form_validation->run() === true) {
			...
			#if empty $id then insert data
			if ($this->service_model->update_admission_service($postData)) {
					$this->session->set_flashdata('message', display('update_successfully'));
				} 
				else 
				{
				...
				redirect('billing/service/extra_service');
	...
	public function services($aid)
	{
		$this->load->model('patient_model');
		$data['services'] = $this->patient_model->patient_all_service($aid);
		$data['content'] = $this->load->view('billing/service/extra_list',$data,true);
		$this->load->view('layout/main_wrapper',$data);
	}
