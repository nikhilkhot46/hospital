    ...
    {   
        ...
        if ($this->form_validation->run() === true) {
            ...
            if ($check_user->num_rows() === 1) {
                if ($check_user->row()->user_role != 10) {
                    if($check_user->row()->expire_date < date("Y-m-d")){
                        $this->session->set_flashdata('exception',display('license_expire'));
                        redirect('login');
                    }
                }
                ...
                $this->session->set_userdata([
                    'isLogIn'   => true,
                    'user_id' => (($check_user->row()->user_role==10)?$check_user->row()->id:$check_user->row()->user_id),
                    'patient_id' => (($check_user->row()->user_role==10)?$check_user->row()->patient_id:null),
                    'email'     => $check_user->row()->email,
                    'fullname'  => $check_user->row()->firstname.' '.$check_user->row()->lastname,
                    'user_role' => (($check_user->row()->user_role==10)?10:$check_user->row()->user_role),
