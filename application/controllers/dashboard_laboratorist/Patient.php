...
class Patient extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
			'dashboard_laboratorist/patient_model',
			'doctor_model',
            'document_model' 
		));
	...
	{ 
		...
		$data['content'] = $this->load->view('dashboard_laboratorist/patient',$data,true);
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('mobile', display('mobile'),'required|regex_match[/^[0-9]{10}$/]');
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($postData['id'])) {
				...
				redirect('dashboard_laboratorist/patient/profile/' . $patient_id);
			} else {
				...
				redirect('dashboard_laboratorist/patient/edit/'.$postData['id']);
			}

		} else {
			$data['content'] = $this->load->view('dashboard_laboratorist/patient_form',$data,true);
			$this->load->view('dashboard_laboratorist/main_wrapper',$data);
		} 
	}


	public function profile($patient_id = null, $getid = null)
	{ 
        ...
        $data['status'] = (object)$postData = array( 
            'id'              => $this->input->post('id'),
            'cm_patient_id'   => $this->input->post('patient_id'),
		...
		$this->load->model('case_manager/status_model');
		if ($this->form_validation->run()) {
			...
			$data['statuss'] = $this->status_model->read($patient_id);
			$data['content'] = $this->load->view('dashboard_laboratorist/patient_profile',$data,true);
			$this->load->view('dashboard_laboratorist/main_wrapper',$data);
		} 
	}
	...
	{ 
		...
		redirect('dashboard_laboratorist/patient');
	}
    ...
    */
	...
	{ 
		...
		$data['content'] = $this->load->view('dashboard_laboratorist/document',$data,true);
		$this->load->view('dashboard_laboratorist/main_wrapper',$data);
	} 
    ...
    {  
        ...
        /*-----------CREATE A NEW RECORD-----------*/
        if ($this->form_validation->run() === true) { 
			$result = $this->patient_model->read_by_patient_id($this->input->post('patient_id'));
			if($result){
            ...
            redirect('dashboard_laboratorist/patient/document_form');
        } else {
            $data['doctor_list'] = $this->doctor_model->doctor_list(); 
            $data['content'] = $this->load->view('dashboard_laboratorist/document_form',$data,true);
            $this->load->view('dashboard_laboratorist/main_wrapper',$data);
        }  
    } 
