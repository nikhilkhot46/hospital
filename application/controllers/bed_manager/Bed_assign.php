...
class Bed_assign extends CI_Controller {
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() === true && $this->check_bed(true) === true) { 
            ...
            for ($i = 0; $i <= $numberDays; $i++) {
                ...
                if(!$result || empty($result) || $result == null || $result == NULL || $result == ""){
            ...
            $data['bed_list'] = $this->bed_model->bed_list();
            $data['patient_list'] = $this->patient_model->read();
            $patients = [];
            foreach ($data['patient_list'] as $value) {
                $x['id'] = $value->patient_id;
                $x['value'] = "($value->patient_id) - ".$value->firstname." ".$value->lastname;
    ...
    public function tranferbed() 
    {
        ...
        if ($this->form_validation->run() === true && $this->check_bed(true) === true) { 
            ...
            //$this->bed_assign_model->delete($serial);
            //add new data
            $this->bed_assign_model->inactive($newSerial,date('Y-m-d', strtotime($this->input->post('assign_date',true))));
            ...
            $data['bed']      = $this->bed_assign_model->read_by_serial($serial);
            $data['bed_list'] = $this->bed_model->bed_list();
            $data['content']  = $this->load->view('bed_manager/bedAssign',$data,true);
            $this->load->view('layout/main_wrapper',$data);
        }
    }
