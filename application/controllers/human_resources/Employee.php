...
class Employee extends CI_Controller {
	...
	{  
		...
		$expire_date = $this->db->select('expire_date')->from('user')->where('user_id',$admin_id)->get()->row()->expire_date;
        ...
        if (empty($user_id)) {
            ...
            if ($this->form_validation->run() === true) { 
				...
				$data['userRoles'] = $this->user_roles();
				unset($data['userRoles']['1']);
				//print_r($data['userRoles']);exit;
