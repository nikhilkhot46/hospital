            ...
            <div class="col-sm-12">
                <div class="panel panel-bd lobidrag">
                    ...
                    <div class="panel-body">
                                   <?php echo form_open_multipart('purchase_manager/item/purchase_update',array('class' => 'form-vertical', 'id' => 'purchase_update'))?>
                            ...
                            <div class="col-sm-6">
                               <div class="form-group row">
                                   ...
                                   <div class="col-sm-6">
                                        ...
                                        ?>
                                        <option <?= $supplier_selected[0]['supplier_id']==$value->supplier_id?"selected":"" ?> value="<?= $value->supplier_id?>"><?= $value->supplier_name?></option>
                                        <?php
                                                }
                                        ?>
                                        </select>
                                    ...
                                    <div class="col-sm-3">
                                        <a href="<?php echo base_url('supplier/Csupplier'); ?>"><?php echo display('add_supplier') ?></a>
                                    </div>
                                </div> 
                            </div>
                                ...
                                <tbody id="addPurchaseItem">
                                    ...
                                    <tr>
                                        <td class="span3 supplier">
                                           <input type="text" name="product_name" required class="form-control product_name productSelection" onkeypress="product_pur_or_list(<?= $purchase['sl']?>);" placeholder="<?php echo display('item_name') ?>" id="product_name_<?= $purchase['sl']?>" tabindex="5" value="<?= $purchase['item_name'] ?>"  >

                                            <input type="hidden" class="autocomplete_hidden_value product_id_<?= $purchase['sl']?>" name="product_id[]" id="SchoolHiddenId" value="<?= $purchase['item_id'] ?>"/>

                                            <input type="hidden" class="sl" value="<?= $purchase['sl']?>">
                                        </td>
                                       <td class="wt">
                                                <input type="text" id="available_quantity_<?= $purchase['sl']?>" class="form-control text-right stock_ctn_<?= $purchase['sl']?>" placeholder="0.00" readonly/>
                                            </td>
                                        
                                            <td class="text-right">

                                                <input type="text" name="product_quantity[]" id="cartoon_<?= $purchase['sl']?>" class="form-control text-right store_cal_<?= $purchase['sl']?>" onkeyup="calculate_store(<?= $purchase['sl']?>);" onchange="calculate_store(<?= $purchase['sl']?>);" placeholder="0.00" value="<?= $purchase['quantity'] ?>" min="0" tabindex="6"/>
                                            </td>
                                            <td class="test">
                                                <input type="text" name="product_rate[]" onkeyup="calculate_store(<?= $purchase['sl']?>);" onchange="calculate_store(<?= $purchase['sl']?>);" id="product_rate_<?= $purchase['sl']?>" class="form-control product_rate_<?= $purchase['sl']?> text-right" placeholder="0.00" value="<?= $purchase['rate'] ?>" min="0" tabindex="7"/>
                                            </td>
                                           

                                            <td class="text-right">
                                                <input class="form-control total_price text-right" type="text" name="total_price[]" id="total_price_<?= $purchase['sl']?>" value="<?= $purchase['total_amount'] ?>" readonly="readonly" />
                                            </td>
                                            <td>

                                               

                                                <button style="text-align: right;" class="btn btn-danger red" type="button" value="<?php echo display('delete')?>" onclick="deleteRow(this)" tabindex="8"><i class="fa fa-trash"></i></button>
                                            </td>
                                    </tr>
                                    <?php
                                        }
...
var options = {
    ...
    source: function( request, response ) {
        var item_name = $('#item_name_'+sl).val();
    $.ajax( {
      url: "<?php echo base_url('purchase_manager/item/product_search')?>",
      method: 'post',
      dataType: "json",
      data: {
        term: request.term,
   ...
   select: function( event, ui ) {
        ...
        $.ajax({
            type: "POST",
            url: base_url+"purchase_manager/item/get_total_product",
...
$('body').on('keydown.autocomplete', '.item_name', function() {
