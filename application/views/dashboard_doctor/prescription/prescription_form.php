                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 table-responsive">
                        ...
                        <table class="table">
                            <thead>
                                <tr>  
                                    ...
                                    <th width="33.33%">
                                        <ul class="list-unstyled">
                                            <li><input type="text" name="appointment_id" value="<?php echo (!empty($appointment_id)?$appointment_id:null) ?>" class="invoice-input form-control" placeholder="<?php echo display('appointment_id') ?>" readonly></li>
                                            <li><input type="text" name="date" required value="<?php echo date('Y-m-d') ?>" class="datepicker invoice-input form-control" placeholder="<?php echo display('date') ?>"></li> 
                                            <li><input type="text" value="<?php echo $website->title; ?>" class="invoice-input form-control" placeholder="<?php echo display('hospital_name') ?>" disabled></li> 
                                            <li><input type="text" value="<?php echo $website->description; ?>" class="invoice-input form-control" placeholder="<?php echo display('address') ?>" disabled></li> 
                                        </ul>
                                    </th> 
                                </tr> 
                                <tr>
                        ...
                        <table class="table table-striped"> 
                            <thead>
                                <tr class="bg-primary">
                                    <th width="210"><?php echo display('medicine_name') ?></th>
                            ...
                            <tbody id="medicine">
                                <tr>
                                    <td><input type="text" name="medicine_name[]" class="medicine form-control" placeholder="<?php echo display('medicine_name') ?>" ><div id="qty"></div></td>
...
$(document).ready(function() {   
    ...
    // medicine list
    $('body').on('keyup click', '.medicine', function(){
        $(this).autocomplete({
            source: [
                <?php 
    ...
    $('body').on('blur', '.medicine', function(){
        var p = $(this).val();
        var x = p.substr(p.length - 9).replace(")","")
        var item
        $.ajax({
            url:'<?= base_url() ?>dashboard_doctor/prescription/prescription/productStock/'+x,
            data:{
                'product_id':x,
                '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'
            },
            success:function (data){
                $('#medicine tr:last-child').find('#qty').empty();
                $.each( JSON.parse(data), function( key, value ) {
                    content = "<div class='row' style='font-size:12px'><div class='col-md-8'>Batch ID:"+ value.batch_id+" </div><div class='col-md-4'>Qty:" + value.stock+"</div></div>";
                    $('#medicine tr:last-child').find('#qty').append(content);
                });
            },
            error: function(xhr, message,a){
